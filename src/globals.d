...
import std.math, std.algorithm, std.typecons, std.conv, std.zip, std.stdio,
    std.file, std.array;
...
class Globals
{
    ...
    //------------------------------------------------------------------------------------          
    auto calc_target_position(v2f player_pos, float player_speed,
            v2f shooter_pos, int time_in_frames)
    {
...
class Input
{
    ...
    void record()
    {
        ...
        {
            globals.recording.writefln("%s %s %s ", globals.frame, k,
                    Keyboard.isKeyPressed(inputmap[k].key));
        }
    }
