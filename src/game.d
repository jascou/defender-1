...
import starfield, entity, lasers, gameevent, hud, particle, characters,
    behaviours;
...
class Game : Scene
{
    ...
    void update_level()
    {
        ...
        if (number_landers == 0)
        {
            ...
            status = app.LEVEL_END;
            characters.set_string("levelend",
                    format("ATTACK WAVE %s COMPLETED", (app.globals.gamelevel + 1)),
                    v2f(450, 300), justify.LEFT);
    ...
    auto explosion()
    {
        ...
        auto _func = delegate void(Entity obj) {
            ...
            if (obj.on_screen && obj.name != "bullet")
            {
                ...
                auto e = new Emitter(app, obj.explosion_bits, 0,
                        init_explosion(obj), move_explosion());
                e.pos = obj.screenpos;
    ...
    auto player_explosion()
    {
        ...
        auto _func = delegate void(Entity obj) {
            ...
            auto e = new Emitter(app, obj.explosion_bits, 0,
                    init_player_explosion(obj), move_player_explosion());
            e.pos = obj.screenpos;
            particle_system.trigger(e);
        };
        return _func;
    ...
    auto player_died()
    {
        ...
        auto _func = delegate void(Entity obj) {
            ...
            entity_mgr.do_list(["lander", "swarmer", "baiter", "pod"],
                    behaviours.randomize_position());
            entity_mgr.do_list(["human"], (Entity e) {
                e.state = HUMAN_GROUND;
                e.parent = null;
            });
            entity_mgr.do_list(["lander"], (Entity e) {
                if (e.state != LANDER_MUTATED)
                {
                    e.init(e);
                }
            });
    ...
    auto game_over()
    {
        ...
        return () { status = app.GAMEOVER; running = false; };
    }
    ...
    auto sound(string name)
    {
        ...
        auto _func = delegate void() { sound_mgr.play(name); };
        return _func;
    }
    //--------------------------------------------------------------------------------------------------------------------
    ...
    void explode_mountains()
    {
        ...
        {
            if (e.state == LANDER_SEARCHING || e.state == LANDER_DROPPING
                    || e.state == LANDER_GRABBING || e.state == LANDER_ABDUCTING)
            {
