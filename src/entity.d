...
class Entity
{
    ...
    string name;
    App app;
    int id, status, state, life, explosion_bits, xpix, ypix, dispersion, wait,
        soundctr, spawndelay, extent;
    ...
    this(int id, string name, App app, Game game, void delegate(Entity) init_plugin,
    ...
    void draw(RenderTarget rtex)
    {
        ...
        if (status > QUIESCENT && status != RESPAWN)
        {
            ...
            {
                hudshape.fillColor = hudcolor;
                game.hud.draw_entity(hudshape, worldpos, rtex);
            }
        }
    }
    //--------------------------------------------------------------------------------------------------------------------    
