...
import std.random, std.math, std.format;
...
auto star_init(App app)
{
    ...
    auto _initialise = delegate void(Particle p) {
        ...
        p.pos = v2f(uniform(-200F, uniform(0, app.win.size.x) + 200F),
                uniform(160F, app.win.size.y - 200F));
        p.dx = 0;
        p.dy = 0;
        ...
        if (e.dispersion > 4)
        {
            ...
            for (int i = 0; i < e.xpix; i++)
            {
                for (int j = 0; j < e.ypix; j++)
                {
                    ...
                    s.position = e.screenpos + v2f(e.dispersion * (i - e.xpix / 2),
                            e.dispersion * (j - e.ypix / 2));
                    e.app.rtex.draw(s);
                }
            }
        }
...
auto player_init()
{
    ...
    auto _initialise = delegate void(Entity e) {
        ...
        e.sprite2 = e.game.sprite_mgr.get_sprite_ref("thrust");
        e.sprite2.origin = v2f(10, 15);
        e.sprite2.scale = v2f(0.8, 0.8);
        e.offset = v2f(-50, 0);
        e.speed = 0;
        e.direction = 1;
        e.dy = 0;
    ...
    auto _control = delegate void(Entity p) {
        ...
        default:
            ...
            if (p.app.globals.input("REVERSE"))
            {
                ...
                {
                    p.set_sprite("player");
                    p.sprite2 = p.game.sprite_mgr.get_sprite_ref("thrust");
                    p.sprite2.origin = v2f(10, 15);
                    p.sprite2.scale = v2f(0.8, 0.8);
                }
                else
                {
                    p.set_sprite("player_r");
                    p.sprite2 = p.game.sprite_mgr.get_sprite_ref("thrust_r");
                    p.sprite2.origin = v2f(10, 15);
                    p.sprite2.scale = v2f(0.8, 0.8);
                }
            }
...
auto player_draw()
{
    ...
    auto _draw = delegate void(Entity p) {
        ...
        p.app.rtex.draw(p.sprite);
        p.sprite2.position = p.sprite.position + p.offset;
        if (p.thrust)
        {
            p.app.rtex.draw(p.sprite2);
        }
    };
    return _draw;
}
...
auto lander_ai()
{
    ...
    auto _ai = delegate void(Entity p) {
        ...
        case LANDER_SEARCHING:
            ...
            if (y >= p.game.world.get_height_at_pos(cast(int) x) - HOVER_HEIGHT)
                p.dy = -3;
            else if (y < p.game.world.get_height_at_pos(cast(int) x) - HOVER_HEIGHT)
                p.dy = 3;
            break;
    ...
    auto _ai = delegate void(Entity p) {
        ...
        if (p.state == BAITER_SPAWN)
        {
            ...
            if (p.spawndelay == 0)
            {
                ...
                p.status = entity.ALIVE;
                p.worldpos = v2f(p.game.player.worldpos.x + uniform(800,
                        1000) * p.game.player.direction, uniform(400, 600));
                p.game.event_handler.notify(gameevent.MATERIALISE, p);
            }
            else
                return;
...
auto bomber_init()
{
    ...
    auto _initialise = delegate void(Entity p) {
        ...
        p.worldpos = v2f(p.id * 300, 200 + ((p.app.win.size.y / 2 + p.id * 200) % (
            p.app.win.size.y - 200)));
...
auto randomize_position()
{
    ...
    auto inner = delegate void(Entity e) { auto p = randpos(e.app); e.worldpos = p;  };
        return inner;
}
...
auto randcol2()
{
    ...
    return Color(cast(ubyte) uniform(0, 255), cast(ubyte) uniform(0, 255),
